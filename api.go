...
func httpLogin(cfg *config.Config, creds *credentials) bool {
	...
	passwordSha1 := utils.GetSha1(creds.Password)

	cnt := 0

	db.QueryRow("SELECT COUNT(*) FROM account WHERE username = ? AND password = ?", creds.Username, passwordSha1).Scan(&cnt)
...
func apiStart(br *broker) {
	...
	r.POST("/signup", func(c *gin.Context) {
		...
		passwordSha1 := utils.GetSha1(creds.Password)
		_, err = db.Exec("INSERT INTO account values(?,?,?)", creds.Username, passwordSha1, isAdmin)
		if err != nil {
			log.Error().Msg(err.Error())
			c.Status(http.StatusInternalServerError)
			return
